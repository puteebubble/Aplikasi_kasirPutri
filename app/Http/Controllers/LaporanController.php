    ...
    public function bulanan(Request $request)
{
    $bulan = $request->bulan;
    $tahun = $request->tahun;
    $pelangganId = $request->pelanggan_id; // Terima ID pelanggan dari request
    ...
    $penjualan = Penjualan::leftJoin('users', 'users.id', 'penjualans.user_id')
        ...
        ->select(
            ...
            'pelanggans.nama as nama_pelanggan' // Ubah users menjadi pelanggans
        )
        ->whereMonth('penjualans.tanggal', $bulan)
        ->whereYear('penjualans.tanggal', $tahun)
        ->where('penjualans.status', 'selesai');

    // Filter berdasarkan ID pelanggan jika ID pelanggan tersedia
    if ($pelangganId) {
        $penjualan->where('penjualans.pelanggan_id', $pelangganId);
    }

    // Group by tanggal dan nama pelanggan
    $penjualan = $penjualan->groupBy('tgl', 'nama_pelanggan')->get(); // Ubah nama_kasir menjadi nama_pelanggan
