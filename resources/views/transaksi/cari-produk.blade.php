...
@push('scripts')
    <script>
        ...
        function fetchCariProduk(search) {
            ...
            function(response) {
                ...
                if(response.length === 0) {
                    const message = `<tr><td colspan="2" class="text-center">Produk tidak ditemukan.</td></tr>`;
                    $('#resultProduk').append(message);
                } else {
        ...
        function addResultProduk(item) {
            ...
            const row = `<tr><td>${nama_produk}</td><td class="text-right">${btn}</td></tr>`;
